...
function displayPages($current, $max) {
	echo '<a href="./?site='.$_GET['site'].'&section='.$_GET['section'].'&page=1"'.isSelected($current, 1).'>1</a> ';
	if (($current - 3) > 1) echo '...';

	for ($i = -2; $i <= 2; $i++) {
		if ((($current + $i) > 1) && (($current + $i) < $max)) {
			echo '<a href="./?site='.$_GET['site'].'&section='.$_GET['section'].'&page='.($current + $i).'"'.isSelected($current, ($current + $i)).'>'.($current + $i).'</a> ';
		}
	}

	if (($current + 3) < $max) echo '...';
	echo '<a href="./?site='.$_GET['site'].'&section='.$_GET['section'].'&page='.$max.'"'.isSelected($current, $max).'>'.$max.'</a> ';
}
?>
